...
# ===============================================
# Script: vm-service-status.sh
# Description:
#   - Ensures a local directory exists
#   - Retrieves systemd service status report
#   - Saves the report locally
#   - Zips the local directory and saves it as a .zip file
#   - Deletes the local-reports directory after zipping
#   - Moves all ZIP files to a zipArchive directory
# ===============================================
...
reportFile="$localDir/$(hostname)_service_status.txt"

echo "=============================================="
echo "         SERVICE STATUS REPORT - $(hostname)"
echo "==============================================" > "$reportFile"
...

{
    ...
    systemctl list-units --type=service --state=running --no-pager --no-legend | awk '{print $1 "\t" $4}' | sort
    echo

    echo "------ STOPPED SERVICES ------"
    systemctl list-units --type=service --state=inactive --no-pager --no-legend | awk '{print $1 "\t" $4}' | sort
    echo

    echo "------ FAILED SERVICES ------"
    systemctl list-units --type=service --state=failed --no-pager --no-legend | awk '{print $1 "\t" $4}' | sort
    echo
...
fi
{
    ...
    echo "------ STOPPED SERVICES ------"
    stopped_services=$(systemctl list-units --type=service --state=inactive --no-pager --no-legend | awk '{print $1}')
    for service in $stopped_services; do
        echo "⏹️ $service is stopped. Attempting to start up to 3 times..."
        attempt=1
        success=0
...
# ---- Move existing ZIPs to archive ----
echo "Moving ZIP files from $zipFolder to $zipArchive..."
shopt -s nullglob  # Avoid errors if no .zip files are present
for zipFile in "$zipFolder"/*.zip; do
    mv "$zipFile" "$zipArchive/"
    echo "Moved $(basename "$zipFile") to $zipArchive"
done
